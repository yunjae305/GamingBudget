...
- 상단 카드 첫 줄(`#todayRow`)에 "오늘 쓸 수 있는 돈" 이 가장 먼저 보이고(누르면 예산 탭), 예산 탭 맨 위 `#todayCard`(`paintToday()`)에 큰 숫자로. 초과면 빨강, 남으면 초록. **이 줄은 내역·예산 탭에서만 보인다** — 통계·자산·배분 탭은 수입/지출/저축 카드만(사용자 결정, 2026-09-24).
...
- 탭 위의 큰 제목(h1 "가계부/예산/통계…")은 **없앴다**(2026-09-24). 상단은 달 이동 바 → 카드 순서. "오늘 쓸 수 있는 돈" 줄은 내역·예산 탭에서만.
