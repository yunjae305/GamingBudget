...
- 총 예산은 직접 적거나, 배분 탭에서 고른 봉투 금액을 "총 예산으로 넣기" 로 보낸다. (예산 계산 카드는 배분과 겹쳐 2026-09-24 뺌.)
...
- 봉투(배분 탭): **슬라이더 없이 기입식**(2026-09-24). 실수령액을 적고 봉투마다 "%" 또는 "원" 을 골라 숫자만 넣으면 막대·남은 돈이 바로 뜬다(`envPct`/`envAmt`, `mode`). 합계 100% 이내로 제한 — 넘치는 입력은 남은 몫까지만 받고 토스트. 고른 봉투(`plan.budgetEnv`, 기본 "생활비")를 예산 탭 총 예산으로 보내는 버튼.
