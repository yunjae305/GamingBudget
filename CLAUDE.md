...
- 봉투(배분 탭): **슬라이더 없이 기입식**(2026-09-24). 실수령액을 적고 봉투마다 "%" 또는 "원" 을 골라 숫자만 넣으면 막대·남은 돈이 바로 뜬다(`envPct`/`envAmt`, `mode`). 합계 100% 이내로 제한 — 넘치는 입력은 남은 몫까지만 받고 토스트. 예산 봉투는 `plan.budgetEnv` 또는 이름에 "생활" 이 들어간 봉투(`budgetEnv()`), 그 금액이 자동으로 총 예산. 보내기 버튼은 없다.
